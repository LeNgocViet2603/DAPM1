...
<div class="content-wrapper">
   ...
   <section class="content">
            ...
            <form>
               <div class="row pb-4">
                  <div class="col col-4 cbb-filter">
                     <div class="form-item">
                        ...
                        <select class="cbb-input" name="service" id="">
                           ...
                           <option value="{{$item->maLoaiCSKD}}" <?php if (isset($_GET['service']) && $_GET['service'] == $item->maLoaiCSKD) echo 'selected' ?>>{{$item->tenLoaiCSKD}}</option>
                           @endforeach
                        </select>
                  ...
                  <div class=" col col-4 cbb-filter">
                     <div class="form-item">
                        ...
                        <select class="cbb-input" name="status" id="">
                           <option value="0">Tất cả</option>
                           <option value="inactive" <?php if (isset($_GET['status']) && $_GET['status'] == 'inactive') echo 'selected' ?>>Không vi phạm</option>
                           <option value="active" <?php if (isset($_GET['status']) && $_GET['status'] == 'active') echo 'selected' ?>>>Vi phạm</option>
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="col col-4 cbb-filter">
                     <div class="form-item">
                        ...
                        <select class="cbb-input" name="district" id="district">
                           ...
                           <option value="{{$item->maQuanHuyen}}" <?php if (isset($_GET['district']) && $_GET['district'] == $item->maQuanHuyen) echo 'selected' ?>>{{$item->tenQuanHuyen}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="col col-4 btn-filter">
                     ...
                     <button class="btn btn-primary" type="submit" id="submit-search" style="background-color: #ffc107;margin-right: 10px;"><a href="{{route('statistics.statistics')}}" style="color: black;">Reset</a></button>
      ...
      <script>
         const data = {
            labels: [
               'Nhà hàng ăn uống (<?php echo round($data['countpie1'] * 100 / $data['countstore']);
                                    echo '%' ?>)',
               'Cửa hàng ăn uống (<?php echo round($data['countpie2'] * 100 / $data['countstore']);
                                    echo '%' ?>)',
               'Chợ (<?php echo round($data['countpie3'] * 100 / $data['countstore']);
                     echo '%' ?>)',
               'Sản Phẩm Cung Ứng (<?php echo round($data['countpie4'] * 100 / $data['countstore']);
                                    echo '%' ?>)',
            ],
